...
class report_web extends Event
{
    ...
    public function defaultAction()
    {
        ...
        $results_count = $this->db->query("SELECT count(*) as `count` FROM `result_counting`;")->fetch(PDO::FETCH_ASSOC);
        ...
        # 收錄資料統計
        $results = $this->db->query("SELECT * FROM `result_counting` ORDER BY `count` DESC LIMIT $start,$per")->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function delete()
    {
        ...
        $this->adminHelper->log('次數統計；已刪除結果項目 '.$hash);
    ...
    public function bulk_delete()
    {
        ...
        $this->adminHelper->log('次數統計；已刪除結果項目 '.sizeof($response['success']) . ' 個');
