...
if last_run != None:
	...
	FK_Foreman.throwGrabWork(db, FK_CONFIGS["fetch.depth"]) 
       ...
       db.execute("INSERT INTO `logs` SET `daemon` = 'FOREMAN', `message` = '因為離上次執行的時間還沒有大於間隔值，所以放棄';")

# 如果完全沒跑過
else:
    db.execute("INSERT INTO `logs` SET `daemon` = 'FOREMAN', message = 'invoke FK_Foreman to throw grabbing work', time = NOW();")
    FK_Foreman.throwGrabWork(db , FK_CONFIGS["fetch.depth"])
